...
class Arrays
{
     ...
     * $output = \LianzhCommon\Helper\Arrays::normalize($input);
     * // $output 现在是一个数组，结果如下：
     * // $output = [
     * //   'item1',
     * //   'item2',
     ...
     * @code php
     * $arr = ['', 'test', '   '];
     * \LianzhCommon\Helper\Arrays::remove_empty($arr);
     *
     * dump($arr);
     * // 输出结果中将只有 'test'
     * @endcode
     ...
     *     ['id' => 2, 'value' => '2-1'],
     * ];
     * $values = \LianzhCommon\Helper\Arrays::cols($rows, 'value');
     *
     * dump($values);
     *   // 输出结果为
     *   // [
     ...
     *     ['id' => 2, 'value' => '2-1'],
     * ];
     * $hashmap = \LianzhCommon\Helper\Arrays::hashmap($rows, 'id', 'value');
     *
     * dump($hashmap);
     *   // 输出结果为
     *   // [
     ...
     *     ['id' => 2, 'value' => '2-1'],
     * ];
     * $hashmap = \LianzhCommon\Helper\Arrays::hashmap($rows, 'id');
     *
     * dump($hashmap);
     *   // 输出结果为
     *   // [
     ...
     * ];
     * $values = \LianzhCommon\Helper\Arrays::group_by($rows, 'parent');
     *
     * dump($values);
     *   // 按照 parent 分组的输出结果为
     *   // array(
     ...
     * ];
     *
     * $tree = \LianzhCommon\Helper\Arrays::tree($rows, 'id', 'parent', 'nodes');
     *
     * dump($tree);
     *   // 输出结果为：
     *   // [
     ...
     * @code php
     * $refs = null;
     * $tree = \LianzhCommon\Helper\Arrays::tree($rows, 'id', 'parent', 'nodes', $refs);
     * 
     ...
     * ];
     *
     * $rows = \LianzhCommon\Helper\Arrays::sort_by_col($rows, 'id', SORT_DESC);
     * dump($rows);
     * // 输出结果为：
     * // [
     * //   ['id' => 6, 'value' => '6-1', 'parent' => 3],
     ...
     * 用法：
     * @code php
     * $rows = \LianzhCommon\Helper\Arrays::sort_by_cols($rows, [
     *     'parent' => SORT_ASC, 
     *     'name' => SORT_DESC,
     * ]);
     * @endcode
